...
public struct Star: Hashable, Equatable, @unchecked Sendable {
    ...
    public mutating func loadInfo(starManager: StarManager) async {
        self.info = await starManager.starInfo(forId: id)
    }
    
    /// Create a new star with detailed information loaded
    public func withInfo(starManager: StarManager) async -> Star {
        var star = self
        await star.loadInfo(starManager: starManager)
        return star
    }
}
