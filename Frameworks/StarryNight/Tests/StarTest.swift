...
final class StarTest: XCTestCase {
    ...
    func testStarsWithMaximumMagnitude() async throws {
        // Test with very low magnitude (should only get very bright stars)
        let veryBrightStars = await starManager.stars(maximumMagnitude: 1.0)
    ...
    func testStarsWithModerateMagnitude() async throws {
        // Test with moderate magnitude (should get most of brightest 300)
        let moderateStars = await starManager.stars(maximumMagnitude: 5.0)
    ...
    func testStarsWithHighMagnitude() async throws {
        // Test with high magnitude (should trigger H3 fallback)
        let faintStars = await starManager.stars(maximumMagnitude: 8.0)
    ...
    func testStarsMaximumMagnitudeNoDuplicates() async throws {
        // Test that there are no duplicate stars when combining brightest + H3 levels
        let stars = await starManager.stars(maximumMagnitude: 7.0)
    ...
    func testStarsMaximumMagnitudeComparison() async throws {
        // Test that a more restrictive magnitude returns fewer or equal stars
        let stars5 = await starManager.stars(maximumMagnitude: 5.0)
        let stars6 = await starManager.stars(maximumMagnitude: 6.0)
    ...
    func testClosestStar() async throws {
        ...
        let maybeNorthStar = await starManager.closestStar(
        ...
        let properName = await starManager.starInfo(forId: northStar.id)?.properName
        XCTAssertEqual(properName, "Polaris", "Expected star at north pole to be Polaris")
    }
    
    func testClosestStarSouthPole() async throws {
        ...
        let displayName5 = await starManager.starInfo(forId: star5.id)?.displayName
        let displayName6 = await starManager.starInfo(forId: star6.id)?.displayName
