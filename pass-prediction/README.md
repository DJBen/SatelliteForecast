# SatelliteForecast backend

This folder contains the Firebase push notification service, satellite prediction
API, scheduled jobs, and orbital cache previously maintained in
`DJBen/pass-prediction`. It is now part of the `DJBen/SatelliteForecast` repository.

Run backend commands from this folder (`cd pass-prediction` from the repository
root). Firebase configuration remains in `firebase.json` and `.firebaserc`; the
Firebase / Google Cloud project is still `pass-prediction`. Pass
`--project pass-prediction` explicitly when using Firebase or gcloud commands.

- Push handlers: `functions/main.py` (`notify` and `notify_prominent`).
- Localized push copy: `functions/common/description.py`.
- Orbital cache setup and tests: [orbital_functions/README.md](orbital_functions/README.md).
- Scheduled jobs: `jobs/`.
...
which includes the `sun-moon-transit` branch, plus the existing uncommitted transit
notebook, transit helper changes, and `functions/common/geo_utils.py`. Git history
and both original working directories were backed up separately before removal.
Python virtual environments and caches are not imported; recreate environments
from the relevant requirements files when needed.

The original Cloud Run template documentation follows.
