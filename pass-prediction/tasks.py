...
def build(c):  # noqa: ANN001, ANN201
    ...
    c.run(
        ...
        f"image={REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/samples/pass-prediction:manual"
    )
...
def deploy(c):  # noqa: ANN001, ANN201
    ...
    c.run(
        "gcloud run deploy pass-prediction "
        f"--image {REGION}-docker.pkg.dev/{GOOGLE_CLOUD_PROJECT}/samples/pass-prediction:manual "
        f"--platform managed --region {REGION}"
    )

@task(pre=[require_venv_test])
def test_satellite(c):  # noqa: ANN001, ANN201
    """Run unit tests"""
    with c.prefix(venv):
        c.run("pytest test/test_satellite.py -v -s")

@task(pre=[require_venv_test, test_satellite])
