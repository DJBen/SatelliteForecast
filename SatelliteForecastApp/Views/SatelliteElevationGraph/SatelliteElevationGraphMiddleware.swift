...
extension EffectMiddleware where
    ...
    static var satelliteElevationGraph: SimpleEffectMiddleware<SatelliteElevationGraphAction, AppState> {
        SimpleEffectMiddleware<SatelliteElevationGraphAction, AppState>.onAction { action, _, getState in
            switch action {
            case let .requestRasterizeElevationGraph(size, noradIndex, julianDateRange, traitCollection):
                return .promise(token: "") { context, sink in
                    DispatchQueue.global(qos: .userInitiated).async {
                        ...
                        guard let snapshots = state.navigationState.selectedNoradIndex.flatMap({ state.elementsPropagatorResources.satelliteTrails[$0] })?.snapshots else {
                            return
                        }
